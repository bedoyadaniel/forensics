...
# Dictionary of files generated by the MVT-androidqf tool

This document contains information about the files generated by the MVT through the [mvt-check-androidqf.](https://github.com/mvt-project/mvt/tree/main/src/mvt/android/modules/androidqf) component. The objective of this dictionary is that the analyst has the facility to search for specific information and to know the format in which the forensic analysis information is presented.

This resource is **part of a repository of technical documentation** that aims to establish a proven, flexible and accessible knowledge base to **promote consensual forensic analysis for the benefit of civil society**. To organize the contents, the [Diataxis technical documentation framework](https://diataxis.fr/) is used.

This particular resource falls under the [references](https://diataxis.fr/reference) category, and contains information on the analysis of acquisition output generated by Android devices using the ***mvt-android check-andoridqf*** command during the use of the MVT tool (*Mobile Verification Toolkit)*, developed and maintained by the [Amnesty International Security Lab](https://securitylab.amnesty.org/es/) and belonging to the [MVT Project](https://github.com/mvt-project/). This is so that an analyst person **knows what files are generated, how to use them, where to look for specific information and in what format they will find it.

MVT stands for *Mobile Verification Toolkit*. This is a tool developed, launched and maintained by the [Amnesty International Security Lab](https://securitylab.amnesty.org/es/) belonging to the [MVT Project](https://github.com/mvt-project/).  The intent of this tool is in essence, to facilitate consensus forensic analysis of Android and iOS devices for the purpose of identifying traces of compromise.
...
Each of the categories in turn contains specific files generated by the tool, which are listed in the **contents index** below:
...
During the execution of the ***mvt-android check-andoridqf** command,* MVT also executes modules corresponding to ***mvt-android check-bugreport*** and ***mvt-android check-backup,*** as long as there is a bugreport.zip file and a backup.ab file inside the input folder of an androidqf extraction.

In the supplementary reference material [about files generated by the mvt tool when parsing a bugreport](../02-reference-mvt-bugreport-dictionary/index.md) the output of the check-bugreport command and the resulting files are described. Below is the **the module-specific references to the modules executed by the check-androidqf command to parse the bugreport:**.
...
Likewise, we also list the modules executed by ***check-backup***, which are detailed at the end of this resource.  
...
This file is in *json* format and contains information related to the analysis performed on an extraction completed using AndroidQF. It contains the following information:
...
This information allows to establish a reference of the analyzed file and the indicators tools used in the analysis, this facilitates the [forensic extraction custody process](https://forensics.socialtic.org/explainers/01-explainer-introduccion-forense-digital/01-explainer-introduccion-forense-digital.html#cadena-de-custodia).
...
**information contained** **information contained** **information contained** **information contained** **information contained**

This file is in plain text with a *.log* extension and contains detailed logs of the execution of the *mvt-android check-androidqf* command.

Its contents list the analysis performed on an output folder of a forensic data extraction performed with AndroidQF, including the detection of indicators of compromise to identify malware-related security alerts.

The archive logs are presented in a structured manner with:

* Timestamps
* Running module names
Type of message (*INFO*, *DEBUG*, WARNING, *ERROR*) * Corresponding module action (file parsing, IoCs loading, comparison, and comparison result)
...
* WARNING* Corresponds to activity alerts or suspicious information that an analyst must verify.  
* ERROR: Error messages in some action performed during the analysis, for example when loading a corrupted file or a problem with the execution of the corresponding code.
...
File structure:** ** File structure:** File structure:** File structure:** File structure:** File structure:** File structure:** File structure
...
**Information Contained** **Information Contained** This file is in *csv* format and stores a timeline of the activity.
This file is in *csv* format and stores a timeline of device activity. This activity is obtained from the execution of the MVT analysis modules and is sorted by time.
...
This file is in *json* format and contains a list of device properties that are extracted from the *getprop.tx*t file generated during an extraction with AndoridQF.
...
Some properties are prefixed with *ro*, indicating that they are read-only properties, or were assigned after device reset. Properties prefixed with *persist* refer to reset-resistant configurations. Some properties will have no prefix, so they start directly with the group to which they belong.
...
This file is in *json* format and contains information about the status of device settings in the *namespaces* *system* (UI and general behavior such as brightness, hue, rotation, etc.), *secure* (sensitive settings that require elevated permissions such as application verification, accessibility settings or ADB status) and *global* (parameters shared between users such as networks, location, roaming, global volumes, etc).

The information is presented in 3 blocks, first Settings system, then Settings secure and finally Settings global. Each setting is presented as a key value pair in the structure:

```json
{setting}:{level}]
```

The values that have each setting have different meanings depending on this setting, but generally it is the following:
...
This information is extracted directly from the settings\_global.txt, settings\_secure.txt and settings\_system.txt files generated during an extraction with AndoridQF.

**Why is it important?

Allows you to identify anomalous settings that could compromise the security, privacy or functionality of the device. Unusual default settings may signal intentional or accidental attempts to modify system behavior.
...
This file is in *json* format and contains information about partitions and file system directories mounted on a device in *and* user space in the Android linux kernel extracted from the *mounts.json* file generated during an extraction with AndoridQF.

The module is responsible for identifying suspicious configurations on /system, /vendor, /product and /system\_ext mount points such as read and write access mode, noatime or nodiratime timestamps, subsequently it is responsible for formatting each mount point as follows:
...
* **filesystem_type**: File system type, in android they are the following:
    ...
    * functionfs: System used for [USB gadget] interfaces (https://source.android.com/docs/core/permissions/usb-hal) when Android acts as a USB device.
...
The mount points allow to identify the **storage areas mounted on the device**, their origin and how they are configured. With this information it is possible to know the permissions with which the partitions were mounted, the associated users and in some cases the SELinux security labels applied. This is useful for **identification of mounts that may be suspicious** of malicious activity on the device, for example, a system partition such as /system, /vendor, /product partitions whose mount is usually read-only is mounted with write permissions or lacking security labels or a partition such as /data is mounted with read permissions on subfolders containing application information could indicate unauthorized fraudulent mounts evidencing highly suspicious behavior.
...
This file is in *json* format and contains information about the applications installed on the device. This information is extracted from the *packages.json* file generated during AndroidQF acquisition.
...
This file is in *json* format and contains a list of binaries related to rooting on the device extracted from the r*oot_binaries.json* file generated during an extraction with AndoridQF.

This module is responsible for extracting the binaries from the base file in order to later be able to identify suspected rooting binaries and their traces. The format applied is as follows:
...
This file has relevance to identify files of interest in a forensic investigation, including potential files used by attackers to compromise a device and to perform timely analysis to track malicious activity.
...
In general terms, the backup SMS and MMS message information is searched and decrypted (if necessary) in order to decompress this backup, parse dates, detect links and homologate formats.
